...
export default function PriceTableInsertModal({ open, onClose, onInsert }: Props) {
  ...
  const [slots, setSlots] = useState<Slot[]>(() =>
    Array.from({ length: clamp(count) }, makeEmptySlot),
  );
  ...
  const selectSuggestion = useCallback(
    async (slotIdx: number, s: SuggestItem) => {
      // 캐시 히트
  ...
  const onSlotKeyDown = (slotIdx: number): React.KeyboardEventHandler<HTMLInputElement> => {
    return (e) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        updateSlot(slotIdx, {
          activeIndex: (slot.activeIndex - 1 + slot.suggestions.length) % slot.suggestions.length,
        });
      } else if (e.key === 'Enter') {
        // 후보 열려있으면 선택, 아니면 삽입
        e.preventDefault();
        if (slot.dropdownOpen && slot.suggestions.length > 0) {
          const pick =
            slot.suggestions[Math.max(0, Math.min(slot.activeIndex, slot.suggestions.length - 1))];
  ...
  const allSelectedCount = useMemo(() => slots.filter((s) => !!s.selected).length, [slots]);
      ...
      <div onKeyDown={onModalKeyDown}>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            ...
            <div style={{ fontSize: 12, color: '#64748b' }}>↑↓ 이동 / Enter 선택 / Esc 닫기</div>
          </div>
                      ...
                      <div style={{ marginTop: 6, display: 'flex', gap: 10, alignItems: 'center' }}>
                        {slot.loading ? (
                          ...
                          <span style={{ fontSize: 12, color: '#334155', fontWeight: 700 }}>
                            mode: <span style={{ color: '#0f172a' }}>{selected.mode}</span> /
                            prices: <span style={{ color: '#0f172a' }}>{selected.prices?.length ?? 0}</span>개
                          </span>
                        ) : (
