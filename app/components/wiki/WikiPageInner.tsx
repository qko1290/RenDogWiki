...
export default function WikiPageInner({ user }: Props) {
  ...
  useEffect(() => {
    ...
    // ✅ 이미 같은 문서가 열려있으면 fetchDoc 재호출하지 않음
    const alreadyOpen =
      selectedDocTitle === titleParam &&
      equalsPath(
        selectedDocPath,
        pathParam === '0'
          ? []
          : categoryIdToPathMap[Number(pathParam)] ?? null
      );

    if (alreadyOpen) return; // 🔥 여기서 차단

    const openByIdIfFound = (isRoot: boolean, id?: number) => {
      if (id != null) {
        fetchDoc(isRoot ? [] : [], titleParam, id, {
